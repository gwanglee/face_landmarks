...
def lannet(inputs, is_training=True, deopout_keep_prob=0.5, scope=None):
  end_points = {}
  ...
  with tf.variable_scope(scope, 'lannet', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.max_pool2d], padding='SAME'):
        ...
        net = end_points['conv6'] = slim.conv2d(net, 128, [2, 2], padding='VALID', scope='conv5')
        net = slim.flatten(net)
        net = slim.fully_connected(net, 68*2, activation_fn=None, scope='fc6')

        return net, end_points

def arg_scope(weight_decay=0.0):
  with slim.arg_scope(
